 ...
 * @param value
 */
function flatten(prefix_path, value) {

 prefix_path = prefix_path.split(PATH_SEPARATOR).filter(function(part) { return part.length > 0}).join(PATH_SEPARATOR);
 if ((prefix_path ||"").length) {
  var obj = {};
  obj[prefix_path] = value;
  return flat.flatten(obj, {delimiter: PATH_SEPARATOR});
 }
 else {
  return flat.flatten(value, {delimiter: PATH_SEPARATOR});
 }
}
...
function unflatten(branch_path, flattened) {
 ...
 var unflat = flat.unflatten(flattened, {delimiter: PATH_SEPARATOR});
 var parts = branch_path.split(PATH_SEPARATOR).filter(function(part) { return part.length > 0; });
 var part;
...
function pluck(obj, fields) {
 ...
 return object_filter(obj, function(value, key) { return fields.indexOf(key) >= 0})
}
...
function formatJSON(obj, options) {
 ...
 else if (!options.hasOwnProperty('array_keys') && Object.keys(obj).every(function(key) { return key.indexOf('_') == 0})) {
  // Output as array
